    ...
    <aside class="admin-sidebar" id="sidebar">
        ...
        <nav class="admin-nav">
            @php
                ...
                if (request()->has('q') && mb_strlen(request()->get('q'), 'UTF-8') >= 2)
                    $activeTab = 'section-search';
                elseif (request()->has('agendas'))
                    $activeTab = 'section-events';
                elseif (request()->has('videos'))
                    $activeTab = 'section-videos';
                elseif (request()->has('avis'))
                    $activeTab = 'section-avis';
                elseif (request()->has('allocutions'))
                    $activeTab = 'section-allocutions';
              @endphp
            <a href="#actus" class="nav-btn {{ $activeTab == 'section-actus' ? 'active' : '' }}"
                data-target="section-actus"><i class="fas fa-newspaper"></i> Actualités</a>
            <a href="#events" class="nav-btn {{ $activeTab == 'section-events' ? 'active' : '' }}"
                data-target="section-events"><i class="fas fa-calendar-alt"></i> Agenda</a>
            <a href="#videos" class="nav-btn {{ $activeTab == 'section-videos' ? 'active' : '' }}"
                data-target="section-videos"><i class="fas fa-video"></i> Vidéos</a>
            <a href="#avis" class="nav-btn {{ $activeTab == 'section-avis' ? 'active' : '' }}"
                data-target="section-avis"><i class="fas fa-balance-scale"></i> Avis rendus</a>
            <a href="#allocutions" class="nav-btn {{ $activeTab == 'section-allocutions' ? 'active' : '' }}"
                data-target="section-allocutions"><i class="fas fa-microphone-alt"></i> Allocutions</a>
    ...
    <main class="admin-main">
        <header class="admin-topbar">
            ...
            <form action="{{ route('dashboard') }}" method="GET"
                class="position-relative flex-grow-1 mx-3 d-none d-md-block" style="max-width: 400px;">
                <input type="text" name="q" value="{{ request('q') }}" id="adminGlobalSearch"
                    class="form-control rounded-pill bg-light border-0 px-4 py-2"
                    placeholder="Rechercher (Actualités, Avis, Événement...)" autocomplete="off">
                <button type="submit" class="border-0 bg-transparent position-absolute text-muted"
                    style="right: 15px; top: 50%; transform: translateY(-50%); padding: 0;"><i
                        class="fas fa-search"></i></button>
                <div id="adminSearchResults" class="dropdown-menu w-100 shadow-lg border-0 mt-2 position-absolute"
                    style="display:none; max-height: 400px; overflow-y:auto; z-index:1000; border-radius: 12px;"></div>
            </form>
        ...
        <div class="admin-content">
            ...
            <section id="section-actus" class="admin-section {{ $activeTab == 'section-actus' ? 'active' : '' }}">
                <div class="mb-2 text-uppercase fw-bold text-muted" style="font-size: 0.75rem; letter-spacing: 1px;">
                    Gestion des contenus</div>
                ...
                <h2 class="h4 mb-4 fw-bold" style="color:#003366">Gérer les <span
                        style="color:#007fff">Actualités</span></h2>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle admin-table-mobile">
                            <thead class="table-light">
                                <tr>
                                    <th class="d-none d-md-table-cell" style="cursor:pointer"
                                        onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort' => 'date_publication', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}'">
                                        Date
                                        {!! request('sort') == 'date_publication' ? (request('direction') == 'asc' ? '<i class="fas fa-sort-up ms-1"></i>' : '<i class="fas fa-sort-down ms-1"></i>') : '<i class="fas fa-sort ms-1 opacity-25"></i>' !!}
                                    </th>
                                    <th style="cursor:pointer"
                                        onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort' => 'titre', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc']) }}'">
                                        Titre
                                        {!! request('sort') == 'titre' ? (request('direction') == 'asc' ? '<i class="fas fa-sort-up ms-1"></i>' : '<i class="fas fa-sort-down ms-1"></i>') : '<i class="fas fa-sort ms-1 opacity-25"></i>' !!}
                                    </th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                @if($post->image_url)
                                                    <img src="{{ $post->image_url }}" alt="" class="rounded me-3 img-cover"
                                                ...
                                                <div>
                                                    ...
                                                    <span class="fw-bold text-dark"
                                                        style="line-height: 1.3; display: block;">{{ $post->titre }}</span>
                                                    <div class="d-md-none mt-1">
                                                        <span class="badge bg-light text-primary border"
                                                            style="font-size: 0.65rem;">{{ $post->categorie }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0"
                                                    ...
                                                    <li>
                                                        <a class="dropdown-item text-dark" href="javascript:void(0)"
                                                            onclick="openViewModal('post', {{ $post->id }})">
                                                            <i class="fas fa-eye me-2 text-primary"></i> Voir
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="dropdown-item text-dark" href="javascript:void(0)"
                                                            onclick="openEditModal('post', {{ $post->id }})">
                ...
                <div class="admin-card">
                    ...
                    <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" name="titre"
                                    ...
                                    placeholder="Entrez le titre principal..." required>
                        ...
                        <div class="mt-3">
                            ...
                            <textarea name="resume" class="form-control @error('resume') is-invalid @enderror" rows="2"
                                placeholder="Un court résumé pour la liste..." required>{{ old('resume') }}</textarea>
                        ...
                        <div class="mt-3">
                            ...
                            <textarea id="create-editor-main" name="contenu"
                                class="form-control @error('contenu') is-invalid @enderror" rows="6"
                                placeholder="Introduction ou corps principal...">{{ old('contenu') }}</textarea>
                        </div>

                        <hr class="my-4">
                        <h5 class="text-muted mb-3"><i class="fas fa-plus-circle me-2"></i>Sections & Images
                            Supplémentaires (Optionnelles)</h5>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6 border-end">
                                ...
                                <textarea id="create-editor-s1" name="section_1" class="form-control mb-2" rows="3"
                                    placeholder="Texte additionnel...">{{ old('section_1') }}</textarea>
                                <!-- <label class="form-label small text-muted mt-2">Image 2</label>
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea id="create-editor-s2" name="section_2" class="form-control mb-2" rows="3"
                                    placeholder="Texte additionnel...">{{ old('section_2') }}</textarea>
                                <label class="form-label small text-muted mt-2">Image 3</label>
                            ...
                            <div class="col-md-6 border-end mt-3">
                                ...
                                <textarea id="create-editor-s3" name="section_3" class="form-control mb-2" rows="3"
                                    placeholder="Texte additionnel...">{{ old('section_3') }}</textarea>
                                <label class="form-label small text-muted mt-2">Image 4</label>
                            ...
                            <div class="col-md-6 mt-3">
                                ...
                                <textarea id="create-editor-s4" name="section_4" class="form-control mb-2" rows="3"
                                    placeholder="Texte additionnel...">{{ old('section_4') }}</textarea>
                                <label class="form-label small text-muted mt-2">Image 5</label>
                                <input type="file" name="image_url_5" class="form-control form-control-sm">
                            </div>

                            <!-- Section 5 + Image 6 -->
                            <div class="col-md-6 border-end mt-3">
                                <label class="form-label fw-bold">Section 5</label>
                                <textarea id="create-editor-s5" name="section_5" class="form-control mb-2" rows="3"
                                    placeholder="Texte additionnel...">{{ old('section_5') }}</textarea>
                                <label class="form-label small text-muted mt-2">Image 6</label>
                                <input type="file" name="image_url_6" class="form-control form-control-sm">
                            </div>

                            <!-- Section 6 (texte seul) -->
                            <div class="col-md-6 mt-3">
                                <label class="form-label fw-bold">Section 6</label>
                                <textarea id="create-editor-s6" name="section_6" class="form-control mb-2" rows="3"
                                    placeholder="Texte de conclusion...">{{ old('section_6') }}</textarea>
                                <small class="text-muted">Section texte sans image associée</small>
                            </div>
                        </div>

                        <button type="submit" class="btn px-4 py-2 fw-bold text-white mt-5"
                            style="background: #003366; border-radius: 8px;">
            ...
            @if(isset($searchResults))
                <section id="section-search" class="admin-section {{ $activeTab == 'section-search' ? 'active' : '' }}">
                    <div class="mb-4 d-flex align-items-center justify-content-between">
                        <h2 class="h4 mb-0 fw-bold" style="color:#003366">Résultats pour la recherche <span
                                style="color:#007fff">"{{ request('q') }}"</span></h2>
                        <a href="{{ route('dashboard') }}" class="btn btn-outline-secondary btn-sm"><i
                                class="fas fa-times me-2"></i>Effacer</a>
                    </div>
                        ...
                        <div class="row">
                                ...
                                <div class="col-12 mb-4">
                                    <div class="admin-card">
                                        <div class="admin-card-header bg-light mb-0 border-bottom p-3">
                                            <h4 class="h5 mb-0 fw-bold"><i class="fas fa-newspaper me-2 text-primary"></i>Actualités
                                                ({{ $searchResults['posts']->count() }})</h4>
                                        </div>
                                        <div class="p-3">
                                            <ul class="list-group list-group-flush">
                                                @foreach($searchResults['posts'] as $item)
                                                    <li
                                                        ...
                                                        <button onclick="openEditModal('post', {{ $item->id }})"
                                                            class="btn btn-sm btn-outline-warning"><i class="fas fa-edit me-1"></i>
                                                            Modifier</button>
                                                    </li>
                                                @endforeach
                                            </ul>
                            ...
                            @if($searchResults['avis']->isNotEmpty())
                                <div class="col-12 mb-4">
                                    <div class="admin-card">
                                        <div class="admin-card-header bg-light mb-0 border-bottom p-3">
                                            <h4 class="h5 mb-0 fw-bold"><i class="fas fa-balance-scale me-2 text-primary"></i>Avis
                                                ({{ $searchResults['avis']->count() }})</h4>
                                        </div>
                                        <div class="p-3">
                                            <ul class="list-group list-group-flush">
                                                @foreach($searchResults['avis'] as $item)
                                                    <li
                                                        ...
                                                        <button onclick="openEditModal('avi', {{ $item->id }})"
                                                            class="btn btn-sm btn-outline-warning"><i class="fas fa-edit me-1"></i>
                                                            Modifier</button>
                                                    </li>
                                                @endforeach
                            ...
                            @if($searchResults['agendas']->isNotEmpty())
                                <div class="col-12 mb-4">
                                    <div class="admin-card">
                                        <div class="admin-card-header bg-light mb-0 border-bottom p-3">
                                            <h4 class="h5 mb-0 fw-bold"><i class="fas fa-calendar-alt me-2 text-primary"></i>Agenda
                                                ({{ $searchResults['agendas']->count() }})</h4>
                                        </div>
                                        <div class="p-3">
                                            <ul class="list-group list-group-flush">
                                                    ...
                                                    <li
                                                        ...
                                                        <div>
                                                            ...
                                                            <div class="text-muted small mt-1"><i
                                                                    class="fas fa-map-marker-alt me-1"></i> {{ $item->lieu }}</div>
                                                        </div>
                                                        <button onclick="openEditModal('agenda', {{ $item->id }})"
                                                            class="btn btn-sm btn-outline-warning"><i class="fas fa-edit me-1"></i>
                                                            Modifier</button>
                                                    </li>
                                                @endforeach
                            ...
                            @if($searchResults['videos']->isNotEmpty())
                                <div class="col-12 mb-4">
                                    <div class="admin-card">
                                        <div class="admin-card-header bg-light mb-0 border-bottom p-3">
                                            <h4 class="h5 mb-0 fw-bold"><i class="fas fa-video me-2 text-primary"></i>Vidéos
                                                ({{ $searchResults['videos']->count() }})</h4>
                                        </div>
                                        <div class="p-3">
                                            <ul class="list-group list-group-flush">
                                                @foreach($searchResults['videos'] as $item)
                                                    <li
                                                        class="list-group-item d-flex justify-content-between align-items-center py-3 border-bottom">
                                                        <span class="fw-bold">{{ $item->title }}</span>
                                                        <button onclick="openEditModal('video', {{ $item->id }})"
                                                            class="btn btn-sm btn-outline-warning"><i class="fas fa-edit me-1"></i>
                                                            Modifier</button>
                                                    </li>
                                                @endforeach
                            ...
                            @if($searchResults['allocutions']->isNotEmpty())
                                <div class="col-12 mb-4">
                                    <div class="admin-card">
                                        <div class="admin-card-header bg-light mb-0 border-bottom p-3">
                                            <h4 class="h5 mb-0 fw-bold"><i
                                                    class="fas fa-microphone-alt me-2 text-primary"></i>Allocutions
                                                ({{ $searchResults['allocutions']->count() }})</h4>
                                        </div>
                                        <div class="p-3">
                                            <ul class="list-group list-group-flush">
                                                @foreach($searchResults['allocutions'] as $item)
                                                    <li
                                                        class="list-group-item d-flex justify-content-between align-items-center py-3 border-bottom">
                                                        <span class="fw-bold">{{ $item->titre }}</span>
                                                        <button onclick="openEditModal('allocution', {{ $item->id }})"
                                                            class="btn btn-sm btn-outline-warning"><i class="fas fa-edit me-1"></i>
                                                            Modifier</button>
                                                    </li>
                                                @endforeach
...
`;
    ...
    <form action="/admin/posts/${item.id}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="resume" class="form-control" rows="2">${(item.resume || '').replace(/<[^>]*>?/gm, '')}</textarea>
        </div>
        ...
        <div class="p-3 border rounded bg-light mb-3">
            <h6 class="fw-bold border-bottom pb-2"><i class="fas fa-plus-circle me-2"></i>Sections Additionnelles (6 max)</h6>
            ...
            <div class="row">
                <div class="col-md-6 mb-3 border-end">
                    ...
                    ${item.image_url_2 ? `<small class="text-success"><i class="fas fa-check me-1"></i>Image actuelle : <a href="${item.image_url_2}" target="_blank">voir</a></small>` : ''} -->
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    ${item.image_url_3 ? `<small class="text-success"><i class="fas fa-check me-1"></i>Image actuelle : <a href="${item.image_url_3}" target="_blank">voir</a></small>` : ''}
                </div>
                ...
                <div class="col-md-6 mb-3 border-end">
                    ...
                    ${item.image_url_4 ? `<small class="text-success"><i class="fas fa-check me-1"></i>Image actuelle : <a href="${item.image_url_4}" target="_blank">voir</a></small>` : ''}
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label small fw-bold">Section 4 & Image 5</label>
                    <textarea id="editor-s4" name="section_4" class="form-control mb-1">${item.section_4 || ''}</textarea>
                    <input type="file" name="image_url_5" class="form-control form-control-sm mt-2">
                    ${item.image_url_5 ? `<small class="text-success"><i class="fas fa-check me-1"></i>Image actuelle : <a href="${item.image_url_5}" target="_blank">voir</a></small>` : ''}
                </div>

                <div class="col-md-6 mb-3 border-end">
                    <label class="form-label small fw-bold">Section 5 & Image 6</label>
                    <textarea id="editor-s5" name="section_5" class="form-control mb-1">${item.section_5 || ''}</textarea>
                    <input type="file" name="image_url_6" class="form-control form-control-sm mt-2">
                    ${item.image_url_6 ? `<small class="text-success"><i class="fas fa-check me-1"></i>Image actuelle : <a href="${item.image_url_6}" target="_blank">voir</a></small>` : ''}
                </div>

                <div class="col-md-6 mb-3">
                    <label class="form-label small fw-bold">Section 6 (Texte seul)</label>
                    <textarea id="editor-s6" name="section_6" class="form-control mb-1">${item.section_6 || ''}</textarea>
                    <small class="text-muted">Dernière section, sans image</small>
                </div>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            ${item.image_url ? `<small class="text-success mt-1 d-block"><i class="fas fa-check me-1"></i>Image actuelle : <a href="${item.image_url}" target="_blank">voir</a></small>` : ''}
        </div>
            ...
            <i class="fas fa-save me-2"></i>Mettre à jour l'actualité
                    ...
                    <form action="/admin/agendas/${item.id}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="editor-agenda" name="summary" class="form-control" rows="3">${item.summary || ''}</textarea>
                        </div>
                    ...
                    <form action="/admin/videos/${item.id}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="editor-video" name="description" class="form-control" rows="3">${item.description || ''}</textarea>
                        </div>
                        <button type="submit" class="btn btn-primary w-100">Mettre à jour</button>
                    </form>`;
        ...
        <div class="mb-3">
            <label class="form-label">Résumé</label>
            <textarea id="editor-avi" name="resume" class="form-control" rows="2">${item.resume || ''}</textarea>
        </div>
            ...
            <i class="fas fa-save me-2"></i> Enregistrer les modifications
                    ...
                    <form action="/admin/allocutions/${item.id}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="editor-allocution" name="description" class="form-control" rows="3">${item.description || ''}</textarea>
                        </div>
                    ...
                    // 4. Initialisation des éditeurs CKEditor
                    if (type === 'post') {
                        const editors = ['#editor-main', '#editor-s1', '#editor-s2', '#editor-s3', '#editor-s4', '#editor-s5', '#editor-s6'];
                        ...
                        const el = document.querySelector('#editor-agenda');
                        if (el) ClassicEditor.create(el, { toolbar: ['bold', 'italic', 'link', 'bulletedList', 'numberedList', 'undo', 'redo'] }).catch(error => console.error('Erreur CKEditor Agenda', error));
                    } else if (type === 'video') {
                        const el = document.querySelector('#editor-video');
                        if (el) ClassicEditor.create(el, { toolbar: ['bold', 'italic', 'link', 'bulletedList', 'undo', 'redo'], placeholder: 'Ajoutez une brève description ou le contexte de cette vidéo...' }).catch(error => console.error('Erreur CKEditor Video', error));
                    } else if (type === 'avi') {
                        const el = document.querySelector('#editor-avi');
                        if (el) ClassicEditor.create(el, { toolbar: ['heading', '|', 'bold', 'italic', 'link', '|', 'bulletedList', 'numberedList', 'blockQuote', '|', 'undo', 'redo'] }).catch(error => console.error('Erreur CKEditor Avis', error));
                    } else if (type === 'allocution') {
                        const el = document.querySelector('#editor-allocution');
                        if (el) ClassicEditor.create(el, { toolbar: ['bold', 'italic', 'link', 'bulletedList', 'undo', 'redo'] }).catch(error => console.error('Erreur CKEditor Allocution', error));
                    }
                })
                .catch(error => {
