...
<head>
  ...
  <title>{{ $avisshow->titre }} | Conseil Économique et Social</title>
    ...
    <nav class="site-nav" id="mainNav" aria-label="Navigation principale">
      <ul class="site-nav-menu">
        ...
        <li class="site-nav-item has-dropdown">
          <a href="{{ url('avis') }}" class="site-nav-link active" aria-expanded="false" aria-haspopup="true">
    ...
    <div class="col-lg-10">
      <div class="prose">
        @if($avisshow->image_url)
            <img src="{{ $avisshow->image_url }}" alt="{{ $avisshow->titre }}" class="w-100 mb-4 rounded" style="object-fit:cover; max-height:500px;">
        @endif
        
        <div class="d-flex align-items-center mb-4 gap-3 text-muted small border-bottom pb-3">
            <span class="badge bg-danger text-uppercase">{{ $avisshow->commission }}</span>
            <span><i class="far fa-calendar-alt"></i> Publié le {{ \Carbon\Carbon::parse($avisshow->created_at)->translatedFormat('d F Y') }}</span>
        </div>

        @if($avisshow->resume)
        <blockquote class="my-4" style="font-size: 1.15rem; color: var(--bleu-fonce); border-left: 4px solid var(--jaune); background: var(--bleu-clair); padding: 20px; border-radius: 8px;">
            <i class="fas fa-quote-left mr-2 text-primary opacity-50 mb-2"></i><br>
            {{ $avisshow->resume }}
        </blockquote>
        @endif
        ...
        @if($avisshow->pdf_url)
        <div class="mt-5 p-4 rounded text-center" style="background-color: #f8f9fa; border: 1px dashed #ced4da;">
            <a href="{{ $avisshow->pdf_url }}" target="_blank" class="btn btn-primary btn-lg px-5 py-3 shadow-sm" style="background-color: var(--bleu-fonce); border:none;  font-size:1.1rem; border-radius: 12px; transition: transform 0.2s;">
              <i class="fas fa-file-pdf mr-2" style="color:var(--jaune);"></i> Voir le document de l'Avis
            </a>
            <p class="text-muted mt-2 small">S'ouvre dans un nouvel onglet</p>
        </div>
        @endif
        
      </div>
      <div class="mt-5">
        <a href="{{ url('avis') }}" class="btn btn-outline-primary"><i class="fas fa-arrow-left"></i> Retour aux avis</a>
      </div>
    </div>
  </div>
</div>
