...
<div class="row g-4">
    @forelse($actualites as $post)
        <div class="col-md-4">
            <div class="actu-card h-100 reveal">
                @if($post->image_url)
                    <img src="{{ $post->image_url }}" alt="{{ $post->titre }}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                @endif
                
                <div class="actu-img-ph" style="{{ $post->image_url ? 'display:none' : 'display:flex' }}">
                ...
                <div class="actu-body">
                    ...
                    <span class="badge bg-primary mb-2">{{ $post->categorie }}</span>
                    <h3>{{ $post->titre }}</h3>
                    <p class="small text-muted">{{ Str::limit($post->resume, 100) }}</p>
                    <a href="{{ url('actualites/'.$post->slug) }}" class="actu-link">Lire plus <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
        </div>
    @empty
